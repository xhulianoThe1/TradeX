  ...
  Highcharts.stockChart('container', {
    rangeSelector: {
      selected: 4
      	...
      	title: {
		text: "Total Portfolio Opening Price Line Chart for " + "<?php echo $_SESSION['nameOfPortfolio'] ?>"
	},
    yAxis: {
      labels: {
        formatter: function() {
...
$.each(names, function(i, name) {
  ...
  $.getJSON('https://www.quandl.com/api/v3/datasets/WIKI/' + name.toLowerCase() + '.json?start_date=2014-05-01&column_index=1&auth_token=W8yzMDsJZ_TEcrPjWxGn', function(data) {
    var date = [],
		tot = [],
		combine = [],
		temp = [],
	...
	if(seriesCounter == 0){
		value = tot;
		for(k=0; k < tot.length-1;k++){		
			value[k] += (NumberOfStocks*(Number(tot[k]))-Number(tot[k]));
		}
	}
	else if(tot.length > value.length){
		temp = value;
		value = tot;
		for(k=0; k < tot.length-1;k++){		
			value[k] += (NumberOfStocks*(Number(tot[k])))+temp[k]-Number(tot[k]);
		}
	}
	else{
		for(k=0; k < tot.length-1;k++){		
			value[k] += NumberOfStocks*(Number(tot[k]));
		}
	}
    ...
    if (seriesCounter === names.length) {
		for(j=0; j < tot.length-1;j++){
			combine[j] = [D[j], value[j]];
		}
		combine.reverse();
		total = combine;
      createChart();
